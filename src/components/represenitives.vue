<template>
  <div class="rep-cont">
    <div class="rep-db-cont">
      <div class="header">
        <h1>Search By Represenitives</h1>
...
export default {
  ...
  methods: {
    pushToDetailPage(index) {
      console.log(this.persons[index].displayName, this.persons[index]);
      const name = this.persons[index].displayName;
      const person = this.persons[index];
      this.$router.push({
        name: "SingleRep",
        params: { displayName: name, person: person }
      });
    }
  }
};
